...
const useData = () => {
  ...
  const filterUnwantedData = (data) => {
    ...
    return data.filter((item) => {
      ...
      if (item.resource && /\.(ico|png|jpg|jpeg|gif|svg)$/i.test(item.resource))
        return false;

      // Filtrer les requêtes de monitoring/analytics
      if (item.url && /analytics|tracking|monitor|beacon/i.test(item.url))
        return false;

      // Filtrer les objets vides ou sans propriétés essentielles
      if (!item.text && !item.content && !item.message && !item.comment)
        return false;
  ...
  const fetchFeedbacks = async () => {
    try {
      ...
      let feedbacksData = Array.isArray(response)
        ? response
        : response.data || response.feedbacks || [];
      ...
      SentryLogger.logInfo("Feedbacks loaded and filtered successfully", {
        ...
        first_feedback_keys:
          feedbacksData.length > 0 ? Object.keys(feedbacksData[0]) : [],
      });
