...
import { Box, Container, Grid, CircularProgress, Tabs, Tab, Button } from "@mui/material";
...
const Dashboard = () => {
  ...
  // Monitoring Sentry
  useEffect(() => {
    SentryLogger.logInfo("Dashboard component mounted", { component: "Dashboard" });
 ...
 if (loading) {
  ...
  if (error) {
    ...
    SentryLogger.logError(new Error(errorMessage), {
      ...
      error_type: errorMessage.includes("Unexpected token") ? "json_parse_error" : "general_error",
      is_html_response: errorMessage.includes("<!doctype") || errorMessage.includes("<html"),
